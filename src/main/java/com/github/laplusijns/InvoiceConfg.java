...
public class InvoiceConfg {
	...
	ChatModel chatModel() {
		final var openAiChatOptions = OpenAiChatOptions.builder()
				.responseFormat(ResponseFormat.builder().type(Type.JSON_OBJECT).build()).model(optionsModel)
				.temperature(0.0).apiKey(apiKey).baseUrl(baseUrl).build();
	...
	ServletListenerRegistrationBean<HttpSessionListener> sessionListener() {
		final ServletListenerRegistrationBean<HttpSessionListener> listenerRegBean = new ServletListenerRegistrationBean<>();
